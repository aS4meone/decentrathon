...
import { ArrowLeft, User, Brain, FileText, ChevronRight } from "lucide-react";
...
export default function CombinedModule() {
    ...
    const checkTestResult = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/test_result/${uuid}`
        );
        if (!response.ok) {
  ...
  const formatAnswers = (
    currentAnswers: Record<string, number | number[] | string>
  ): FormattedAnswers => {
    ...
    formatted.selectedAnswerIds = formatted.selectedAnswerIds.filter(
      (id) => id !== formatted.height && id !== formatted.weight
    );
  ...
  const handlePrevious = () => {
    if (currentQuestionIndex > 0) {
      ...
      setCurrentQuestionIndex(
        moduleData[currentModuleIndex - 1].moduleQuestions.length - 1
      );
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
          <h1 className="text-xl sm:text-2xl font-bold text-[#12375F] mb-4">
            Error
          </h1>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700">
                ...
                <p>
                  ...
                  {new Date(testResult.patient_birth_date).toLocaleDateString()}
                </p>
                <p>
                  <span className="font-semibold">Рост:</span>{" "}
                  {testResult.height} см
                </p>
                <p>
                  <span className="font-semibold">Вес:</span>{" "}
                  {testResult.weight} кг
                </p>
                <p>
                  <span className="font-semibold">Телефон:</span>{" "}
                  {testResult.patient_phone_number}
                </p>
                <p>
                  <span className="font-semibold">Дата теста:</span>{" "}
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                  <p className="text-base sm:text-lg font-semibold mb-2 text-gray-700 flex flex-row gap-2 items-center justify-between">
                    <span>Оценка по факторам риска</span>
                    <span className="relative group">
                      <FaCircleQuestion className="cursor-pointer hover:text-blue-600" />
                      <span className="absolute left-1/2 transform -translate-x-1/2 bottom-full mb-2 hidden group-hover:block text-xs bg-gray-700 text-white p-2 rounded shadow-md">
                        This is a fafatoolfafatip message.
                      </span>
                    </span>
                  </p>
                  <p className="text-2xl sm:text-3xl font-bold text-blue-600">
                    {testResult.kfr_points}%
                  </p>
                </div>
                <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                  <p className="text-base sm:text-lg font-semibold mb-2 text-gray-700 flex flex-row gap-2 items-center justify-between">
                    <span>Очки симптомов</span>
                    <span className="relative group">
                      <FaCircleQuestion className="cursor-pointer hover:text-blue-600" />
                      <span className="absolute left-1/2 transform -translate-x-1/2 bottom-full mb-2 hidden group-hover:block text-xs bg-gray-700 text-white p-2 rounded shadow-md">
                  ...
                  <p className="text-2xl sm:text-3xl font-bold text-green-600">
                    {testResult.symptoms_points}
                  </p>
                </div>
                <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                  <p className="text-base sm:text-lg font-semibold mb-2 text-gray-700 flex flex-row gap-2 items-center justify-between">
                    <span>Когнитивный тест</span>
                    <span className="relative group">
                      <FaCircleQuestion className="cursor-pointer hover:text-blue-600" />
                      <span className="absolute left-1/2 transform -translate-x-1/2 bottom-full mb-2 hidden group-hover:block text-xs bg-gray-700 text-white p-2 rounded shadow-md">
                        This is a tooltip message.
              ...
              <div className="grid grid-cols-1 gap-6">
                <div>
                  ...
                  <ul className="list-disc pl-5 space-y-2 text-gray-700">
                    {testResult.recommendation_for_user.map(
  ...
  if (showTest && moduleData.length > 0) {
    ...
    const isLastQuestion =
      currentModuleIndex === moduleData.length - 1 &&
      currentQuestionIndex === currentModule.moduleQuestions.length - 1;

    return (
      <div className="min-h-screen py-4 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto rounded-lg">
          <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-[#12375F]">
            ...
            return (
              <QuestionItem
                ...
                question={{
                  id: questionId,
                  text:
                    subQuestion.subQuestionText || subQuestion.subQuestionType,
                }}
              ...
              <button
                ...
                disabled={
                  ...
                  currentQuestionIndex ===
                    currentModule.moduleQuestions.length - 1
                }
                className="w-full sm:w-auto px-6 py-3 bg-[#12375F] text-white rounded-full font-semibold hover:bg-blue-700 transition-colors"
              >
                Далее
